...
(function () {
  ...
  const processPdfLinks = () => {
    ...
    console.log(
      `[PDF Link Creator] Found ${targetElements.length} target elements. Active blob URLs: ${activeBlobUrls.size}`,
    );
    ...
    targetElements.forEach((td, index) => {
      ...
      if (textContent.startsWith('^TEXT^PDF^Base64^')) {
        ...
        try {
          ...
          blobUrlCount++;
          console.log(
            `[PDF Link Creator] Blob URLs - created total: ${blobUrlCount}, currently active: ${activeBlobUrls.size}`,
          );
